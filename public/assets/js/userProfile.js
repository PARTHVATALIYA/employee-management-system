window.onload = () => {
    ...
    $.ajax({
        method: 'GET',
        url: `http://localhost/practice/userManagement/routes/web.php/checkUser?id=${id}`,
        success: function(result) {
            if (result.status == 401) {
                document.getElementsByClassName("unAuthorized")[0].style.display = "block"
                document.getElementById("text").innerHTML = `${result.status} Not authorized`;
    ...
    const submitButton = document.getElementById("updateData");
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        if (response.status == 200) {
                            alertBox("Updated", response.message, "success", true);
                        } else {
...
const alertBox = (title, text, icon, confirmButton) => {
    Swal.fire({
        title: title,
        text: text,
        icon: icon,
        showConfirmButton: confirmButton,
        showCloseButton: !confirmButton
    }).then((result) => {
